      ...
      <div class="overflow-hidden">
        <div class="float-left size-stitle color-3">
          ...
          <p class="inline-block" v-if="comment">
            ...
            <span class="ml-10 size-stitle">{{
              item.repliedUser && item.repliedUser.userName
            }}</span>
          </p>
          ...
          <span class="inline-block cursor-pointer size-title" v-if="isConsole">
            ...
            >
              <template #reference>
                <i
                  class="el-icon-view"
                  :class="{ 'color-active': !item.publish }"
                ></i>
              </template>
            </el-popconfirm>
          </span>
...
export default class ReplicesList extends Vue {
  ...
  replySubmit(v) {
    this.replyhttp
      .reply(
        this.comment ? this.comment._id : this.currentReply._id,
        v,
        this.currentReply.targetId ? undefined : this.currentReply._id
      )
      .then((res) => {
        this.closeReply();
  ...
  deleteReplices(v) {
    if (v.targetId) {
      ...
      this.replyhttp.deleteReplices(v.commentId, v._id).then((res) => {
        this.$parent.$parent.$parent["isReplied"] = !this.$parent.$parent
          .$parent["isReplied"];
        Message.success("删除成功");
      });
    }
  ...
  publishReplices(v) {
    if (v.targetId) {
      this.replyhttp
        .publishComments(v.targetId, v._id, !v.publish)
        .then((res) => {
          this.$parent.$parent["isReplied"] = !this.$parent.$parent[
            "isReplied"
          ];
