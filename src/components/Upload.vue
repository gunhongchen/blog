<template>
    <div class="upload">
        <div class="el-upload">
            <input :multiple="multiple" ref="fileInput" type="file" name="file" @change="fileChange">
        ...
        <ul class="filelist">
            <li class="p-20 mb-10" v-for="(item, index) in fileList" :key="item.name">
                <div>
                    ...
                    <p class="ml-20" :class="{'color-danger': item.uploadError}">{{item.name}}</p>
                </div>
                <i class="el-icon-close cursor-pointer size-main color-2"
