  ...
  <div class="album-trees">
    <div v-for="(item, index) in fileData" :key="item._id">
      <AlbumTreeSingle
        ...
        @closedfolder="closeFolder"
        @showmodal="showAddModal"
        @delete="deleteData($event, index)"
      ></AlbumTreeSingle>
      <div
      ...
      >
        <div v-for="(child, childIndex) in item.children" :key="child._id">
          <AlbumTreeSingle
            ...
            @closedfolder="closeFolder"
            @showmodal="showAddModal"
            @delete="deleteData($event, childIndex, 1)"
          ></AlbumTreeSingle>
        </div>
      </div>
    </div>
    <div class="text-center" v-if="fileData.length < 1">暂无数据</div>
    <el-dialog
      title="添加图片/图集"
      @closed="modalClosed"
...
export default class AlbumTrees extends Vue {
  ...
  openFolder(v) {
    if (this.opened.indexOf(v._id) < 0) {
  ...
  getAlbum(v) {
    ...
    albumHttp.getAlbum(v._id).then((res: any) => {
      if (this.currentFolder._id) {
        ...
        this.currentFolder["childCount"] = res.length;
        this.currentFolder = {};
      }
      this.loading = false;
    });
